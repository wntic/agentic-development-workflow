...
- A fake the test needs that doesn't exist yet → `fake.md` (write the fake first).
...
1. **Pin PERSISTED state, not the in-memory entity.** Assert the write happened via the fake's `updated` call-record (`assert repo.updated == [id]`) AND read the value back (`(await repo.get_by_id(id)).status == DONE`). The fake returns a COPY and logs updates (`fake.md` Rule 9), so a body that mutates the entity but never calls `update()` **reds**. Asserting on the entity object the handler mutated in place pins nothing (it observes the in-memory mutation, not the persist).
...
- Required fake does not exist in `tests/unit/fakes/` → stop, produce it first via `fake.md`.

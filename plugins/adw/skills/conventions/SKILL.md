...
A datastore's `kind` is a free token (not a closed enum — a fixed list is the same disease as a fixed type map). The profile maps a `kind` to the few things needed to wire a repository **without** knowing the backend's SQL/SDK internals (those live in the bodies written against the backend):

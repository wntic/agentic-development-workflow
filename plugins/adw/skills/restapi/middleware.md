...
- The app shell — CORS, the central `DomainError` handler, lifespan → `bootstrap.md`.
- Logic for **one** route → `endpoint.md` (a thin route over an application handler).
- Authenticating / authorizing a route → `auth-dependency.md` (a FastAPI dependency, **not** a middleware).
- Advertising an HTTP code a route can return → `error-responses.md`.
...
- The concern is for one route, not all → stop, use `endpoint.md` + a handler.
- It needs a domain entity / repository / application handler → stop, that's application logic, not a middleware.
- It authenticates or authorizes a request → stop, use `auth-dependency.md` (a FastAPI dependency on the route).
...
- The middleware introduces an HTTP status with no domain exception behind it → stop, register the code via `error-responses.md` (the middleware-code path).

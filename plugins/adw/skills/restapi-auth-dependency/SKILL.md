...
description: The decision rule for which auth dependency a route attaches — `get_current_user` (any authenticated) versus `require_role(Role.X)` (role-gated) — and the `_: CurrentUser` versus `user: CurrentUser` binding convention. Produces no file.
when_to_use: Deciding a route's auth dependency, or checking that a role-gated route binds and advertises correctly.
paths: src/**/restapi/**
---
...
`Role` is a rank-ordered `StrEnum` in the domain (`domain-enum`); `MEMBER < AGENT < ADMIN` is illustrative only — another app may have a two-tier or differently-named ladder. `user.role.satisfies(required)` returns `true` when the caller's rank meets or exceeds the requirement, and `require_role(Role.<MIN_RANK>)` admits that rank and every higher one. Use the placeholder `Role.<MIN_RANK>` in templates — the concrete member comes from the route's own auth requirement against the app's `Role`, never a fixed `SUPER_ADMIN`.
...
A role-gated route must therefore advertise both 401 and 403 (a `get_current_user`-only route just 401). Advertising the codes is `restapi-error-responses`' job, and follows from the chosen dependency.

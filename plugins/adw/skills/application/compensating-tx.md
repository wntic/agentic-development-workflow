...
This skill produces **no new file** — it shapes a command handler's `execute` body (the form `command.md` writes when compensation is required): the try → undo → re-raise structure around the visible side effect.
...
- The handler performs writes across multiple repositories atomically → `unit-of-work.md` (the patterns nest: compensation outside, UoW inside).
- The handler has no external side effect, only a DB write → no compensation needed; the default `command.md` shape (no `try/except`) suffices.
...
8. **Compensation pairs with `unit-of-work.md` cleanly** — compensation wraps the UoW; the patterns nest: try / async with uow / commit / except / undo / raise.

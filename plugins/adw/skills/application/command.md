...
- Read (get/list/count/search) → `query.md`.
- The mutation includes an external IO step before the DB write (file upload, third-party POST) and needs rollback → still this skill, but the handler body follows `compensating-tx.md`.
- Multiple repositories must update atomically (write + audit log) → still this skill, plus inject an `IUnitOfWork` (see `unit-of-work.md`).
...
5. **No `try/except`, with two sanctioned exceptions.** (a) The compensating-transaction pattern — see `compensating-tx.md`. (b) A **failure-state transition then re-raise**: when the contract requires the aggregate to record that it failed before the error propagates (e.g. a processing pipeline that must persist `status=FAILED` so a later read/retry sees it), the handler may `try: <pipeline> except <Err>: <load-or-mutate>; entity.status = FAILED; await repo.update(entity); raise`. The `except` writes the caller-visible state and **re-raises** (never swallows, never logs-and-re-raises — `python-style` §Logging); chain with `raise ... from exc` (`B904`). Anything beyond these two — translating a `DomainError`, catching to swallow, control-flow via exceptions — is still forbidden (the central handler translates; see hard stops).
...
Follow `architecture` §Python Package Structure to register both modules in the subpackage `__init__.py`. The DI provider that constructs this handler is the responsibility of `infra-integration` `container.md`.

## Hard stops

- Spec asks the handler to return a list, a `Result`, or the entity → stop, use `query.md` (and re-read the spec — mutations don't return data).
- Spec asks the handler to catch a `DomainError` and translate it → stop, that's the central error handler's job (`restapi` `error-responses.md`).
- Spec asks the handler to validate cross-aggregate state inline → stop, extract a `domain-ports` §Domain Service and inject it.
- Spec implies multiple writes must be atomic → stop, request an `IUnitOfWork` dependency (see `unit-of-work.md`).
- Spec implies an external IO step before the DB write → stop, this still uses this skill but the body must follow `compensating-tx.md`.

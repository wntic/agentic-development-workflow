...
3. **No cross-cutting registries.** Adding a new endpoint touches exactly one new test file. The "every route returns 401 unauth" and "every route declares its error codes in OpenAPI" checks are owned by `discovery.md` and derive their inputs from `app.routes` / `app.openapi()` — no `_endpoints()` / `_EXPECTED` tables to extend.
...
6. **In an app that declares auth, `authed_client` is the only sanctioned client.** `async with authed_client(role=Role.<MEMBER>, ...) as client:` — the `async with` form is non-negotiable; bare assignment leaks the ASGI transport. Raw `AsyncClient(transport=ASGITransport(...))` is otherwise reserved for `discovery.md` unauth probes. **Exception — an auth-less app** (no auth declared): there is no `authed_client`; drive the route with a plain `AsyncClient` over `real_app` (the public template above), still via `async with`.

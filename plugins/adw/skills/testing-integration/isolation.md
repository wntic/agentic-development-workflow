...
- The authed `httpx.AsyncClient` factory → `authed-client.md`.
- A repository contract test → `repository-contract.md` (consumes the `sf` fixture this skill provides).
- An API endpoint test → `endpoint.md` (consumes both `sf` and `authed_client`).
- Cross-cutting "every route returns 401 unauth" / "OpenAPI codes match `error_responses(...)`" → `discovery.md`.

...
1. **Every test takes `sf: async_sessionmaker[AsyncSession]`.** The rollback fixture from `isolation.md` makes the DB empty at test start and discards everything at teardown. No marker, no other DB fixture.
...
11. **No FastAPI, no `httpx`, no DI container.** This test imports the repository class, takes `sf`, calls methods, asserts. The HTTP surface is `endpoint.md`.
...
- `tests/integration/conftest.py` missing or `sf` not provided → stop, install `isolation.md` first.

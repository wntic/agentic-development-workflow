...
- Spec asks to fake a handler's **concrete domain-service** dependency (e.g. `QuotaPolicy`, injected as the class, not a Protocol) → stop, a structural fake won't type-check there; **subclass the service** (override the method under test, bypass `__init__`) or have the handler **inject via a Protocol**. This is the handler-test's call — see `test-application-handler`'s rule that a concrete domain service cannot be faked structurally. Repository/capability fakes (this skill) are structural because their dependencies are Protocols.

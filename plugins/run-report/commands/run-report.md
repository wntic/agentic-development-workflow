---
description: "Report on how a run went — the main/orchestrator log AND the subagents it spawned: which ran, how long, what they produced, how the toolchain moved, and every blocker (denied/rejected tool calls) — as structured Markdown, optionally bundled with raw transcripts"
argument-hint: "[session|latest|all] [--include-main|--main-only] [--raw] [--out DIR] [--blob-cap N]"
---

# /run-report [session] [scope-flags]

Generate a structured Markdown report of how a Claude Code run went: the **main/orchestrator
...
1. Decide scope from the request. Default is subagents of the latest session. If the user
   ...
   ```
   python3 "${CLAUDE_PLUGIN_ROOT}/scripts/run_report.py" --cwd "$PWD" --session "${ARGUMENTS:-latest}"
   ```
   ...
   `~/.claude/projects/<slug>/run-reports/<session>/` (or `--out`):
   ...
   **Full archive for later analysis** (main session + its subagents + raw), e.g.:
   ```
   B=~/.claude/projects/<slug>/run-reports/BUNDLE-<label>
   python3 "${CLAUDE_PLUGIN_ROOT}/scripts/run_report.py" --cwd "$PWD" --session <id> --main-only --raw --out "$B/orchestrator"
   python3 "${CLAUDE_PLUGIN_ROOT}/scripts/run_report.py" --cwd "$PWD" --session <sub-session> --include-main --raw --out "$B/subagents"
   ```
...
- If the analyzer path above came through unexpanded (a `python3: can't open file '/scripts/…'`
  ...
  `ls ~/.claude/plugins/cache/*/run-report/*/scripts/run_report.py`
- Reports written before 2026-08-09 live under `agent-reports/`, the directory this plugin used
  when it was called `agent-report`. Nothing migrates them; look there for older bundles.
...
- The analyzer is stdlib-only Python; it reads transcripts and writes Markdown, nothing else.
  Edit `scripts/run_report.py` to change what the report extracts.

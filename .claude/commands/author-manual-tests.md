...
- the stub file path and the **test skill** for its node (read that skill's *Template(s)* + *Rules* —
  ...
  fake's domain methods, inline `_Raise*` repos for failure injection, the `then.with` post-state);
...
to **refute** each assert — would a plausibly-wrong body still pass it? (a no-op "re-save unchanged", a
swapped/again field, a missing state transition, a dummy that returns the happy value, an exception
caught-and-swallowed). Report each assert as **strong** (a wrong body would fail it) or **weak** (passes
but does not pin the contract). A weak assert is **re-authored** (back to step 2's author context to
strengthen it) or, if it cannot be tightened at unit level, **flagged** into the residual review surface.

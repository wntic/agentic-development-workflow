...
- **The validated manifest(s)** — path to the epic `manifest.yaml` (e.g. `examples/helpdesk_manifest.yaml`). Assume it has passed `.claude/tools/validate_manifest.py` — but re-run it yourself as step 1 (cheap, and your contract is "scaffold only a fully-valid, fully-covered manifest"). **App-mode (multi-context, `conventions` block F):** an app may be a SET of context manifests scaffolded into ONE package (contexts as sibling subpackages keyed by `subdomain`). When given several manifests — or one new context manifest plus an output tree that already holds other contexts — validate each with `--app <epics-dir>` so cross-epic refs resolve, then scaffold per block F: per-context artifacts per manifest, the SHARED substrate (`exceptions.py`, the Postgres bootstrap + `DbSettings`, `main.py`, `error_handler.py`, `dependencies.py`, `containers.py`, `pyproject.toml`) emitted ONCE from the UNION of the contexts. A cross-epic ref `<subdomain>:<Name>` is just a cross-subdomain reference — strip the prefix, import `<Name>` from the `<subdomain>` subpackage. Single-manifest is the degenerate case.
...
├── alembic.ini + migrations/  # relational stores only — Alembic config (glue) + write-once 0001 baseline (block C)
...
3. **Bootstrap inventory — emit the COMPLETE set, never a subset.** The working tree decides which run-once skills fire (MANIFEST_SCHEMA principle 4) — on a fresh output everything below runs; on a re-run, skip only what already exists. In **app-mode** (multiple contexts, `conventions` block F) the bootstrap is the shared substrate: emit it from the UNION of all in-scope contexts, not one — and on a re-run that ADDS a context, regenerate each shared file from the union so the new context's contributions are added without dropping the existing ones (e.g. `exceptions.py` = every context's exceptions deduped; `main.py` includes every context's router; `containers.py` wires every context's providers). All of the following are required, none optional:
   - `domain/exceptions.py` — the catalog (entries from every in-scope context's `domain.exceptions`, deduped by name).
   ...
   - **Test bootstrap** (ALL of): the root `conftest.py`; `test-integration-isolation`; `test-integration-authed-client`; and the `test-discovery-invariants` files under `tests/integration/api/`. Emit each only when its invariant has a target in THIS app (graph-derived presence — never freeze a source-app feature as universal, `PRINCIPLES.md` §C6): always `test_openapi_advertises_error_codes.py` and `test_cors.py`; `test_unauth_returns_401.py` **only when the app declares auth** (an authenticated endpoint / token-verifier capability — else there is no `dependencies.py` / `UnauthorizedError` to import and no protected route to probe); `test_info.py` **only when the manifest declares an info/health endpoint** (the REST bootstrap produces none, so absent that node it is NOT emitted — a `GET /info` test against an app with no such route is a frozen-source-app artifact that 404s); `test_request_size_limit.py` **only when a size-cap middleware is declared** (it asserts a 413 that nothing produces otherwise).
...
4. **Walk the graph in dependency order** (the order keeps every contract import resolvable when its file is written — mined from the retired generator's `generate_all`):
   ...
   **infrastructure** → metadata/engine bootstrap (only if a `uses_bootstrap` store backs a repo — the engine/`session_factory` factory rendered COMPLETE per block C, not a scaffold) → **Alembic migrations bootstrap** (same `uses_bootstrap` trigger, block C: `alembic.ini` + `migrations/env.py` + `migrations/script.py.mako` as COMPLETE glue regenerated every run, plus the **write-once** `migrations/versions/0001_initial.py` baseline — emit the baseline **only when `migrations/versions/` has no `*.py` yet**, never clobber a chain with brownfield deltas; without this the integration suite dies at `alembic upgrade head`, the A4 hazard) → repositories (skill is store-profile-dispatched per `conventions` block B/C — `infra-sqlalchemy-repository` for a relational store, `infra-store-repository` for any client-style store) + table scaffolds (relational stores **only** — a client-style store has no SQLAlchemy table) → settings → capability adapters → connection factories (non-bootstrap stores — `create_<datastore>_client` rendered COMPLETE per block C, no `NotImplementedError`) → `__init__`
   **DI** → `containers.py`
   ...
   **resumable**: a file already written and non-empty is done — on a re-run (including a `/scaffold`
   ...
   - **c. Classify** the node: **declarative** (enum without methods, VO/entity without invariants, any protocol, DTO, filter, REST schema, settings without `methods`, the exception catalog) → render the template in **full**, substituting manifest identifiers + derived names + contract imports. **Body-bearing** (any method body — handler, service, repository, adapter, endpoint, enum method, settings `method`, the ASGI `__call__` of a `restapi.middlewares` middleware — plus `__post_init__` invariants and the relational table) → emit the **scaffold form** (see above). **The connection/engine factory is NOT body-bearing** — for a known store profile it is COMPLETE glue (the fixed `create_engine`/`create_session_factory` and `create_<datastore>_client(settings)` forms in `conventions` block C), rendered in full with no `NotImplementedError`; only a genuinely unknown store `kind` leaves it a scaffold. A `NotImplementedError` connection factory maps to no manifest node and would die UNMAPPED in the runner, then crash the DI container at construct while the toolchain stays green (the A4 hazard).
...
5. **Whole-graph glue passes.** After the per-node files exist, emit the files that are pure functions of the graph (read `general-python-package`, `general-imports-conventions`, `infra-di-provider` as needed):
   ...
   - **Middlewares** (`restapi.middlewares`, if any) — wire `app.add_middleware(<Cls>, **config)` in `main.py` **after the bootstrap's CORS, in manifest order** (Starlette wraps **last-added outermost** → the last middleware in the list is the request's outermost layer, and CORS — added first by the bootstrap — sits innermost); re-export each middleware module in `restapi/middleware/__init__.py`, and **import the middleware classes into `main.py` via the collapsed package re-export** — `from .middleware import (<ClsA>, <ClsB>)`, **one line, not per-submodule** (`general-imports-conventions`). Middleware is **not** the routers carve-out: middleware class names are distinct (no colliding symbol), so they ride the normal `__init__.py` re-export + same-package collapse — only `restapi/routers/` is imported per-module-and-aliased (the colliding `router` symbol). Then populate `MIDDLEWARE_ERRORS` in `schemas/errors.py` from the **union of every middleware's `introduces_http`** (regenerated glue — the only place those codes can live, since `errors.py` is overwritten each run); emit `tests/integration/api/test_request_size_limit.py` only when some middleware declares 413.
   - **`pyproject.toml`** — the **substrate ∪ the union of every infra node's `requires_packages`** over the graph (block D), **names only, no versions** (`uv add` pins at install time); src-layout build config; `[tool.pytest.ini_options]` with `asyncio_mode = "auto"` **and** `asyncio_default_fixture_loop_scope = "session"` + `asyncio_default_test_loop_scope = "session"` (the integration conftest's async engine is session-scoped, so the whole run must share ONE event loop — without it a DB-touching integration test whose statement errors crashes at teardown with `RuntimeError: Event loop is closed`, see `test-integration-isolation`); ruff first-party = the package; the `[tool.ruff.lint] select` and the `__init__.py` F403/F405 per-file ignore **per `conventions` block E** (the select includes `B006` + `B904`); **dev dependencies under `[dependency-groups]` `dev = [...]` (PEP 735), never the deprecated `[tool.uv.dev-dependencies]` table** (block D). The Dev list includes `cryptography` (the authed-client integration conftest needs it). The runtime substrate includes `python-multipart` **when any endpoint is `request_kind: multipart`** (FastAPI needs it at `create_app()` for `Form(...)`/`UploadFile` — mypy/ruff/tests miss it; block D's graph-derived gate).
...
6. **Tests (the §9 seam — derive one per artifact, drop NONE).** Tests are derived per artifact, never enumerated in the manifest (MANIFEST_SCHEMA principle 3) — but "derived" means **every** artifact of these kinds gets its test file; do **not** skip a whole category (a run that emits handler tests but no domain tests is a defect). Pick each test skill from `conventions` block B:
   ...
   - **The flat vs manual seam:**
     ...
     - **Manual stub — write-once, implementer fills the assert** (`test_<verb>_<noun>_handler_manual.py`): **otherwise** — multiple dependencies, a `then` of `calls`/`logs`, a `returns` of a `*Result` DTO (list/projection), or relations/time/negative beyond a flat literal. One `@pytest.mark.skip` function per scenario carrying the contract-comment; the implementer writes the assertion later (separate context — anti-collusion). You **own the scenario list** — never silently drop a scenario. **The stub's `# CONTRACT —` comment carries the handler's CONSTRUCTOR SIGNATURE** (the ctor's dependency param names + types, e.g. `# ctor: UploadRecordingHandler(meeting_repo: IMeetingRepository, blob_store: ICanStoreBlobs, quota_policy: QuotaPolicy)`) — derivable from the manifest `handler.dependencies` you already used to write the handler scaffold. Without it a body-blind assert author cannot construct the handler under test and is forced to peek at the body to find the param names (the F-020 anti-collusion leak); with it the author wires the fakes from the contract alone (§9 / D3).
...
7. **Self-verify (the placement safety net — spec §3/§4/E).** Scaffolds compile and type-check even with `NotImplementedError`, so a misplaced file or a wrong signature goes red **before** any implementer runs. Run, from the output root:
   ...
   - **Zero inline `# type: ignore` on content modules.** `grep -rn "# type: ignore" <tree>/src` must return nothing — `conventions` block E sanctions only the project-wide `[[tool.mypy.overrides]] ignore_missing_imports` for a stub-less SDK, never an inline ignore on glue you emit. The recurring offender is `restapi/dependencies.py` `require_role` — carry the gated role on a typed wrapper, not a `# type: ignore[attr-defined]` stash (`restapi-app-bootstrap`'s template). A hit is a placement/typing bug to fix at the source, not suppress.
   - **Zero `from __future__ import annotations`.** `grep -rn "from __future__ import annotations" <tree>/src` must return nothing. `general-typing-conventions` bans it project-wide (the runtime stack — Pydantic / pydantic-settings, `dependency-injector`, dataclass `__post_init__`, FastAPI — introspects annotations, which PEP 563 stringifies, breaking it silently). It is NOT caught by ruff/mypy and is runtime-harmless in a module like `domain/exceptions.py`, so it stays green and invisible — do not emit it as a reflexive module header. A hit is a glue defect to delete at the source.
   ...
   - `uv run mypy …src tests` — green with the `NotImplementedError` bodies in place (the test scaffolds type-check too: flat tests are full, `_manual` stubs are annotated `-> None` skips, fixtures carry full annotations).
...
8. **Freeze the scaffold baseline (the attribution record — spec §4).** Once self-verify is green and *every body is still `raise NotImplementedError`* (the table still column-less), snapshot the tree: `uv run .claude/tools/scaffold_snapshot.py snapshot <output>`. This writes the immutable `<output>.scaffold/` baseline against which `/verify` later diffs each implementer's edit — so a convention slip (e.g. an import reaching into a submodule, a touched glue file) is attributable to scaffolder vs implementer instead of lost the moment a body is filled. Do this **last**, before reporting; never re-snapshot a tree whose bodies are already filled (the tool refuses without `--force` — do not force it).

9. **Report** with this exact format:
...
4. **Declarative/glue is regenerated; body-bearing is written once.** On a re-run you overwrite every declarative/glue file from the manifest, but you **must not** overwrite an existing body-bearing file — the implementer owns it (§4). On a fresh output everything is new; on a re-run, inspect the tree (step 3) and leave filled bodies alone. **App-mode:** a shared-substrate declarative/glue file is regenerated from the UNION of all in-scope contexts (block F) — so adding a second context regenerates `exceptions.py` / `containers.py` / `main.py` from auth ∪ tickets, never from the new context alone (which would drop the first). A cross-epic `<subdomain>:<Name>` reference resolves to an import from the `<subdomain>` subpackage that another context already scaffolded — a normal cross-subdomain edge, no special handling.

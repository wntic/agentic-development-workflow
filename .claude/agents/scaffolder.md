...
5. **Whole-graph glue passes.** After the per-node files exist, emit the files that are pure functions of the graph (read `general-python-package`, `general-imports-conventions`, `infra-di-provider` as needed):
   ...
   - **`pyproject.toml`** — the **substrate ∪ the union of every infra node's `requires_packages`** over the graph (block D), **names only, no versions** (`uv add` pins at install time); src-layout build config; `[tool.pytest.ini_options] asyncio_mode = "auto"`; ruff first-party = the package; the `[tool.ruff.lint] select` and the `__init__.py` F403/F405 per-file ignore **per `conventions` block E** (the select includes `B904`); **dev dependencies under `[dependency-groups]` `dev = [...]` (PEP 735), never the deprecated `[tool.uv.dev-dependencies]` table** (block D). The Dev list includes `cryptography` (the authed-client integration conftest needs it). The runtime substrate includes `python-multipart` **when any endpoint is `request_kind: multipart`** (FastAPI needs it at `create_app()` for `Form(...)`/`UploadFile` — mypy/ruff/tests miss it; block D's graph-derived gate).

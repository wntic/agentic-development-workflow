...
4. **Walk the graph in dependency order** (the order keeps every contract import resolvable when its file is written — mined from the retired generator's `generate_all`):
   ...
   **infrastructure** → metadata/engine bootstrap (only if a `uses_bootstrap` store backs a repo — the engine/`session_factory` factory rendered COMPLETE per block C, not a scaffold) → repositories (skill is store-profile-dispatched per `conventions` block B/C — `infra-sqlalchemy-repository` for a relational store, `infra-store-repository` for any client-style store) + table scaffolds (relational stores **only** — a client-style store has no SQLAlchemy table) → settings → capability adapters → connection factories (non-bootstrap stores — `create_<datastore>_client` rendered COMPLETE per block C, no `NotImplementedError`) → `__init__`
   **DI** → `containers.py`

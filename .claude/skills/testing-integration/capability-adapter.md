...
6. **`assert exc.value.context["<key>"] == <value>` on every translated exception.** This is the capability-adapter analogue of the `context["constraint"]` rule in `repository-contract.md`. The fake-based handler test cannot verify this — only this test can.
...
13. **No rollback assumption.** Unlike Postgres + `sf`, S3 / Redis / Kafka may not roll back. Either the fixture cleans up (preferred — extend `isolation.md`) or each test uses a unique key prefix scoped to the test. Specify which in the spec.
...
- Spec includes FastAPI / `httpx.AsyncClient` over `ASGITransport` / DI container references → stop, that's `endpoint.md`.

...
- **Non-relational repositories.** The producer table maps `infrastructure.repositories` to `infra-sqlalchemy-repository` (SQLAlchemy Core) — correct for a relational store. A repository on a non-bootstrap store (qdrant/redis) has **no dedicated skill yet**: it is a §16 coverage-gap, closed by authoring the missing `infra-<kind>-repository` skill when the first such repository appears.

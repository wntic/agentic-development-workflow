...
2. **Exact ASGI shape.** `__init__(self, app: ASGIApp, <config…>)` stores `app` + the config on `self`; `async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None`. Configuration arrives as constructor keyword arguments (the values passed at `app.add_middleware(Cls, **config)`).

...
Every route that uses an auth dependency must list the matching status codes when `error-responses.md` runs:

- `Depends(get_current_user)` → include `401`.
- `Depends(require_role(...))` → include `401` **and** `403`.

A role-gated route must therefore advertise both 401 and 403 (a `get_current_user`-only route just 401). Advertising the codes is `error-responses.md`' job, derived from the chosen dependency — not a field this skill writes.
...
- Spec proposes a third auth dependency type → stop, the two declared by `bootstrap.md` are exhaustive; introduce a finer-grained authorization rule in the application handler instead.
- A role-gated route advertises `401` but not `403` (via `error-responses.md`) → stop, the advertised codes must match the chosen dependency (`require_role` → 401 **and** 403).

...
- A frozen `@dataclass(frozen=True)` value object → `value-object.md`.
- A `StrEnum` / `Enum` member set → `enum.md`.
- A domain service (orchestrator with injected protocols, or pure-logic service) → `domain-service.md`.
- A grep-firewall architectural rule → `architecture-rule.md`.

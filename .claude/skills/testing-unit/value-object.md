...
- An entity (UUID identity, mutable) → `entity.md`.
- An enum → `enum.md`.
- A service → `domain-service.md`.
- A grep-firewall rule → `architecture-rule.md`.
...
4. **No `_make_*` builder.** Value objects are small — pass fields directly. Builders are for many-field entities (`entity.md`).

...
- **Skills** (`plugins/adw/skills/`) are the **how-to-write-a-component** knowledge. House-style for a given
...
(shape + threshold: `plugins/adw/skills/CONVENTIONS.md`, "Skill format").
...
a skill or an agent/command. *Do:* artifact-shaped → a skill in `plugins/adw/skills/` (via
`meta-skill-author`), four-section body, no orchestration sections; process-shaped → an agent in
`plugins/adw/agents/` or a command in `plugins/adw/commands/`. The directories stay separate on purpose:

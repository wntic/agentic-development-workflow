# UC-11 (refined) — View my workspace and plan

> Refined from `specs/use-cases/UC-11-view-workspace.md` (verbatim, unchanged). Folds in the BA's answers
...
- No product question applied to this UC. The only open item — whether the monthly usage meter
  ("7 of 20 meetings this month") belongs on this panel and who owns the cross-context number — is an
  **architecture** question, not a product one, and stays open in `epic.md`'s "Raised for the architect"
  section for the architect to resolve. It is deliberately **not** folded here as if it were a product
  decision; the source's A2 / Description "tentatively decided / Revisit with the architect" wording is
  preserved verbatim.

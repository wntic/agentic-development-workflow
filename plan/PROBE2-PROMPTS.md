...
**Оговорка, без которой файл вреден.** Промпты 003 и 004 написаны до того, как 001 и 002 существуют. К
моменту, когда до них дойдёт дело, живая спека будет другой — **перечитай их против `specs/` и правь**,
не вставляй буквально. Это не осторожность, а прямая цитата из харнес-поста Anthropic, на котором стоит
...
|---|---|---|
| 001 | бронь на интервал | greenfield-путь целиком, субстрат, `Makefile` |
| 002 | отказ при пересечении | **`domain-service`** — правило поверх двух броней |
...
прогон, читать отчёт `agent-report` (с 2026-08-04 установлена 1.1.2 — считает запуски инструмента, а не
вызовы `Bash`).

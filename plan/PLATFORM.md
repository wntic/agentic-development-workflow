...
снят**. Что происходит с каталогом `skills/`, созданным **после** старта сессии, на дату этого
раздела оставалось не проверено; закрыто 2026-08-10 — вопрос 11 ниже, ровно тем экспериментом,
который здесь был назван (живая сессия на FIFO по образцу вопроса 5, новый каталог между ходами).
...
**Оговорка к строке `skills count` — что считает поле.** Число приведено дословно, как требует
легенда файла, но считает оно **скиллы сессии**: плагинные, проектные и встроенные вместе, — а не
каталог плагина, который читается из дерева (`ls -d plugins/adw/skills/*/ | wc -l`) и с этим полем не
совпадает. Какие источники в него входят и сколько даёт каждый — **НЕ ПРОВЕРЕНО**: мерилось здесь не
это. Совпадение с историческим размером каталога `adw` до пересборки — ловушка чтения, а не признак
того, что сессия читает устаревшую копию: последнее снято выводом самого раздела.
...
## Вопрос 11. Виден ли сессии каталог `skills/`, появившийся после её старта

Замер по решению человека 2026-08-10 (`plan/findings/legacy.md`, «Решения человека, 2026-08-10 —
седьмая чистка (волна S21), прогоны цикла», строка F-272), задача —
`plan/S21-05-probe-post-start-catalog.md`. Закрывает пометку вопроса 9 про каталог `skills/`,
созданный после старта сессии (след F-60), — ровно тем экспериментом, который там назван.
**Дата: 2026-08-10 (+05:00). Версия: `claude --version` → `2.1.226 (Claude Code)`. macOS Darwin
25.3.0.** Все времена внутри раздела — UTC из `date -u` замеряющего (вечер `2026-08-09Z` — это ночь
2026-08-10 в +05:00); зона названа явно, потому что единственная выдуманная загадка этого файла
(вопрос 9) родилась из смешения зон.

**Стенд — `<scratch>/probe10/`, выбрасываемый.** Плагин `postplug` через `--plugin-dir`: агент
`pquery` — `tools: Read, Glob, Grep, Skill`, поля `skills:` нет; на старте прогона А у плагина
**нет каталога `skills/`**, у проекта-пустышки `proj/` есть `.claude/`, но **нет `.claude/skills/`**.
Скиллы вымышленных форматов, укладываемые между ходами:

- `petrel-manifest-parser` — плагинный, `.pet`, маркер `WV5-PETREL-CINDER-2204`;
- `cormorant-roster-parser` — проектный, `.cor`, маркер `RD8-CORMORANT-GRANITE-6106`;
- `gannet-ledger-parser` — для прогона Б, `.gan`, маркер `TK3-GANNET-MOSSROCK-7742`.

Дискриминатор — по образцу вопроса 9: спрошены геометрия записи и маркер, существующие только в
теле скилла; независимый от текста ответа признак — `tool_use Skill` в `stream-json` и строка
`SkillTool returning …` в дебаг-логе. Живая сессия — методика вопроса 5: `claude -p --input-format
stream-json --output-format stream-json --verbose --forward-subagent-text --permission-mode
bypassPermissions --debug-file …`, stdin — FIFO под фоновым держателем, правки файлов — между
ходами, процесс один на все ходы.

**Контроль K0 — проба различает.** Свежий процесс, оба каталога существуют до старта: сабагент
вызвал оба скилла (`SkillTool returning 2 newMessages for skill postplug:petrel-manifest-parser` и
`… for skill cormorant-roster-parser`), финал дословно `PETREL=WV5-PETREL-CINDER-2204` /
`CORMORANT=RD8-CORMORANT-GRANITE-6106`.

### Прогон А — каталоги создаются между ходами живой сессии

| UTC | событие |
|---|---|
| 20:25:48 | старт; в дебаге `Checking plugin postplug: skillsPath=none, skillsPaths=0 paths` и `Loading skills from: … project=[]` |
| 20:26:01 | ход 1 → `PETREL=UNKNOWN / CORMORANT=UNKNOWN`, вызовов `Skill` ноль |
| 20:26:54 | созданы `postplug/skills/petrel-…/SKILL.md` и `proj/.claude/skills/cormorant-…/SKILL.md` |
| 20:27:41 | отправлен ход 2; диспатч сабагента — аттачмент `Sending 16 skills via attachment (initial)` в 20:27:45 |
| 20:27:49 | перезагрузка списка: `[skills] active — switching poll interval to 2000ms`, `Loading skills from: … project=[…/proj/.claude/skills]`, `Loaded 2 unique skills (… user: 1, project: 1 …)`; строка `Checking plugin postplug: skillsPath=none` — **без изменений** |
| 20:27:58 | `Sending 1 skills via attachment (dynamic)` — родителю |
| 20:28:08 | финал хода 2: `PETREL=UNKNOWN / CORMORANT=UNKNOWN`, вызовов ноль; родитель сам дописал: «a `cormorant-roster-parser` skill just became available in this session» |
| 20:29:07 | ход 3, аттачмент `Sending 17 skills via attachment (initial)`; сабагент: `tool_use Skill {"skill": "cormorant-roster-parser"}`, в дебаге `SkillTool returning 2 newMessages for skill cormorant-roster-parser`, финал `PETREL=UNKNOWN / CORMORANT=RD8-CORMORANT-GRANITE-6106` |

Контроль K2, свежий процесс на тех же каталогах: `Checking plugin postplug: skillsPath=exists`,
вызваны оба скилла, оба маркера.

Три вывода, все **ЗАМЕРЕНО**:

- **Проектный `.claude/skills/`, созданный после старта, подхватывается без рестарта** — включая
  свежесозданный **корень** каталога, в отличие от `agents/` (оговорка вопроса 5). Watcher-строка
  лога каталог проекта не называет (`Watching for changes in skill/command directories:
  /Users/egorvorobyev/.claude/skills, /Users/egorvorobyev/.claude/commands...` — дословно, многоточие
  её собственное); подхват сделал опрос по интервалу, видимый строками `[skills] idle/active —
  switching poll interval to 30000/2000ms`.
- **Список скиллов диспатча замораживается на его запуске.** Диспатч, стартовавший в 20:27:45 — за
  4 с до перезагрузки списка, — скилла не увидел; следующий диспатч того же агента в том же процессе
  увидел и вызвал. Согласуется с выводом вопроса 9 о разрешении `tools:` на запуске диспатча.
- **Плагинный `skills/`, созданный после старта у уже загруженного плагина, не подхватывается до
  рестарта.** `skillsPath` разрешается при загрузке плагина (`none` на старте) и при перезагрузках
  списка скиллов не перечитывается: та же строка `skillsPath=none` в 20:27:49, `PETREL=UNKNOWN` во
  всех трёх ходах; свежий процесс тут же дал `skillsPath=exists` и маркер.

### Прогон Б — плагин устанавливается внутрь сессии CLI-командами (форма F-272)

Живая сессия в пустышке без плагина. При живой сессии, из соседнего процесса, дословно с выводом:

```
$ claude plugin marketplace add <scratch>/probe10/mkt
✔ Successfully added marketplace: postmkt (declared in user settings)
$ claude plugin install postplug2@postmkt
✔ Successfully installed plugin: postplug2@postmkt (scope: user)
```

| UTC | событие |
|---|---|
| 20:41:24 | ход 1 → `Agent type 'postplug2:pquery' not found. Available agents: claude, Explore, general-purpose, Plan, statusline-setup` |
| 20:41:49–50 | обе команды установки отработали; `postmkt` и `postplug2@postmkt` видны в реестрах `~/.claude/plugins/*.json` |
| 20:41:51 | сессия: `Detected change to /Users/egorvorobyev/.claude/settings.json`, `Settings changed from userSettings, updating app state` — дальше только перезапись правил разрешений, ни одной строки про плагины |
| 20:42:49 | ход 2, диспатч → тот же `not found` дословно |
| 20:45:02 | ход 3, диспатч (3 м 12 с после установки; в дебаге третий `Agent tool error`) → тот же `not found` |

Контроль K5, свежий процесс в той же пустышке: агент найден, `tool_use Skill {"skill":
"postplug2:gannet-ledger-parser"}`, `SkillTool returning 2 newMessages …`, финал
`GANNET=TK3-GANNET-MOSSROCK-7742`.

**Вывод: ЗАМЕРЕНО.** Плагин, установленный извне во время работы сессии, для этой сессии не
существует целиком — ни агентов, ни скиллов; изменение `settings.json` сессия замечает, но
перезагружает по нему только правила разрешений. Рестарт подхватывает всё.

После замера установка снята (`claude plugin uninstall postplug2@postmkt`, `claude plugin
marketplace remove postmkt`); оба реестра сверены `diff`-ом с бэкапом, снятым до установки, —
расхождение только в `lastUpdated` прежних записей.

### Побочное и аннулированное

- **`extraKnownMarketplaces` в проектном `settings.json` сам по себе маркетплейс не
  зарегистрировал.** Промежуточный стенд писал между ходами проектный `settings.json` формы раздела
  «Второй путь подключения…» (2026-08-05); живая сессия изменение заметила и плагин не подхватила —
  но и **свежий процесс** на тех же файлах агента не нашёл, а в дебаге дал `Skipping orphaned
  enabledPlugins entry postplug2@postmkt: marketplace not registered`. У замера 2026-08-05
  маркетплейс `wntic-adw` одновременно стоял в пользовательском
  `~/.claude/plugins/known_marketplaces.json` (проверено сегодня: стоит и сейчас) — сегодняшнее
  наблюдение тот раздел не правит, но кладёт рядом границу: на `2.1.226` в свежем каталоге одного
  проектного `settings.json` не хватило даже на старте процесса.
- **Одна сессия аннулирована.** Первая попытка прогона Б выброшена: родитель запустил асинхронного
  агента, тот после ответа хода прочитал staging-файл скилла в scratchpad, и маркер попал в контекст
  сессии. Методика ужесточена (синхронный диспатч; запрет Bash/Read/Glob/Grep и запуска других
  агентов в промпте родителя), стенд Б пересобран на свежем маркере, лежащем только в каталоге
  плагина.

### Свод по вопросу 11

Каталог `skills/`, появившийся после старта сессии, — три разных ответа по трём путям:

| путь | подхват без рестарта |
|---|---|
| проектный `.claude/skills/`, включая новый корень | **да**, опросом; виден диспатчам, стартовавшим после перезагрузки списка |
| `skills/` у плагина, уже загруженного `--plugin-dir` | **нет**: `skillsPath` кэширован с загрузки плагина |
| плагин, установленный CLI извне во время сессии | **нет целиком**: ни агентов, ни скиллов |

Для F-272 отсюда следует граница, а не объяснение: как именно плагин попадал в сессию `crawler` и
когда относительно этого стартовали её диспатчи, этот замер не восстанавливал.

**НЕ ПРОВЕРЕНО**, одним блоком — три пункта, у каждого назван добивающий эксперимент: (а) установка
изнутри самой сессии — интерактивный `/plugin`; стенд ставил извне и headless, внутри-сессионная
установка могла бы тянуть за собой перезагрузку (эксперимент: интерактивная сессия, `/plugin` →
установка → диспатч тем же ходом и следующим); (б) что именно требуется `extraKnownMarketplaces`,
чтобы сработать без пользовательской регистрации — запись в `known_marketplaces.json`, доверие
каталога или смена версии `2.1.222 → 2.1.226` (эксперимент: тот же стенд плюс ручная запись в
...
## Версию читает установка, а не публикация: `claude plugin update` пишет семвер в запись установки

Кладётся по решению человека 2026-08-10 (`plan/findings/legacy.md`, «Решения человека, 2026-08-10 —
девятая чистка (волна S23), платформа и плагин», строка F-264), источник — F-264: замер сделан
главной сессией при шипе `1.2.0` и лежал только в теле находки, тогда как по предмету ему место
здесь. **Класс: ЗАМЕРЕНО. Дата наблюдения: 2026-08-07 (`19:04:48Z` = `2026-08-08T00:04:48+05:00`).
Версия платформы на день наблюдения не записана; на день переноса, 2026-08-10, `claude --version` →
`2.1.226 (Claude Code)`.** Времена приведены так, как их пишет реестр, — в `Z`, с переводом в
`+05:00` там, где он нужен; зона названа явно, потому что единственная выдуманная загадка этого файла
(вопрос 9) родилась из смешения зон.

**Правило: версию читает установка, а не публикация.** Что ляжет в поле `version` записи установки,
решает момент установки и то, что в этот момент несёт `plugin.json`; `git push` в запись не пишет
ничего. Директорийный источник семверу не мешает.

**Три снимка `~/.claude/plugins/installed_plugins.json` за один сеанс.**

1. **До `git push origin main`** — исходная запись `adw@wntic-adw` со `scope: project` для
   `~/Projects/adw-probe`: `version` `89ac6e8c8ea9`, `installPath`
   `cache/wntic-adw/adw/89ac6e8c8ea9`.
2. **После пуша** (`eb0c5c0..ec6a11b`, 64 коммита) — файл побайтово тот же, `diff` пуст. Публикация
   записи установки не касается; это воспроизведение замера 2026-08-05.
3. **После `claude plugin update adw@wntic-adw --scope project`** из `~/Projects/adw-probe` — запись
   изменилась целиком. Вывод CLI дословно:

```
Plugin "adw" updated from 89ac6e8c8ea9 to 1.2.0 for scope project. Restart to apply changes.
```

Поля записи после прогона — чтение полей `installed_plugins.json`, значения дословны:

```
version      89ac6e8c8ea9                      →  1.2.0
installPath  cache/wntic-adw/adw/89ac6e8c8ea9  →  cache/wntic-adw/adw/1.2.0
gitCommitSha                                   →  ec6a11b4433ab4057ec916812509c618b498d2b8
lastUpdated                                    →  2026-08-07T19:04:48.647Z
```

Полные значения двух последних полей взяты перечитыванием записи на день переноса: она с тех пор не
двигалась — `lastUpdated` всё ещё момент того самого `update`, а `gitCommitSha` — ровно
запушенная голова `ec6a11b`.

**Третий экземпляр, независимый от пробы.** Запись `scope: local` для `~/Projects/crawler`,
поставленная позже появления ключа `version` (`installedAt 2026-08-07T11:25:07.933Z`), несёт
`version: "1.1.0"` при `installPath cache/wntic-adw/adw/1.1.0` — тот же directory-источник, тот же
кэш-путь, семвер на месте.

**Что этот раздел уточняет.** §13 («`claude plugin update` — замер главной сессии, 2026-07-30») и
раздел «Второй путь подключения плагина…» (2026-08-05) **не правятся**; поправка живёт здесь. Из них
двоих выводился оборот «семвер в запись установки не попадает вовсе» — он опирался на два
наблюдения, и верна из них только первая половина:

- **пуш запись не меняет** — верно, и подтверждено снимками 1–2 выше; это и есть содержание
  четвёртого факта §13 («правка в дереве» ≠ «правка доступна», лечение — `git push`, и только потом
  `update`);
- **у первой пробы в поле `version` стоял SHA** (`version=89ac6e8c8ea9`, цитата записи в разделе
  2026-08-05) — верно как наблюдение, но **неверно** как свойство directory-источника. Причина —
  время установки: пробу ставили `installedAt 2026-07-30T05:15:54.875Z`, а ключ `version` появился в
  `plugins/adw/.claude-plugin/plugin.json` только 2026-08-04 —
  `git log --format='%h %ad %s' --date=short -S '"version"' -- plugins/adw/.claude-plugin/plugin.json`
  → `b67982d 2026-08-04 feat(plugin): the plugin gets a version, and it starts at …`. Пока ключа не
  было, в поле ложился SHA; как только он появился, `update` пишет туда семвер.

**Границы замера.**

- **НЕ ПРОВЕРЕНО:** что попадёт в `version` при установке плагина **без** ключа `version` в
  `plugin.json` сегодня. Случай 2026-07-30 восстановлен по времени установки, а не поставлен
  опытом (эксперимент: выбрасываемый плагин без ключа, `claude plugin install`, чтение записи).
- **НЕ ПРОВЕРЕНО:** что делает `update`, когда семвер в `plugin.json` не изменился, а голова
  маркетплейса ушла вперёд, — двигается ли `gitCommitSha` при неподвижном `version` и остаётся ли
  `installPath` тем же каталогом (эксперимент: второй `update` после коммита без бампа версии).
